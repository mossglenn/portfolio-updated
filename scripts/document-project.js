...
const docsDir = path.join(rootDir, '.reference-files', 'project-docs');
...
async function createDirectories() {
  ...
  await fs.mkdir(path.join(docsDir, 'components'), { recursive: true });
  await fs.mkdir(path.join(docsDir, 'configs'), { recursive: true });
  await fs.mkdir(path.join(docsDir, 'patterns'), { recursive: true });
}
...
async function generateFileStructure() {
  ...
  try {
    ...
    await fs.writeFile(path.join(docsDir, 'file-structure.md'), markdown);
    console.log('File structure documentation generated.');
    
    return { files, filesByExt };
  } catch (error) {
...
async function documentConfigFiles() {
  ...
  let configMarkdown = `# Project Configuration Files\n\n`;
  configMarkdown += `Generated: ${new Date().toLocaleString()}\n\n`;
  ...
  for (const configFile of CONFIG_FILES) {
    try {
      ...
      if (exists) {
        ...
        await fs.writeFile(path.join(docsDir, 'configs', path.basename(configFile)), content);
  ...
  await fs.writeFile(path.join(docsDir, 'configuration.md'), configMarkdown);
  console.log('Configuration files documented.');
}
...
async function analyzeComponentPatterns(filesByExt) {
  ...
  // Sample up to 10 representative components
  const sampleSize = Math.min(10, componentFiles.length);
  ...
  patternsMarkdown += `Generated: ${new Date().toLocaleString()}\n\n`;
  patternsMarkdown += `This document contains ${sampledComponents.length} representative components from your project.\n\n`;
  
  for (const componentFile of sampledComponents) {
    try {
      const content = await fs.readFile(path.join(rootDir, componentFile), 'utf8');
      
      // Create a hash of the file content for the filename
      const hash = createHash('md5').update(componentFile).digest('hex').substring(0, 8);
      const sampleFileName = `${path.basename(componentFile, path.extname(componentFile))}-${hash}${path.extname(componentFile)}`;
      
      // Save the component
      await fs.writeFile(path.join(docsDir, 'components', sampleFileName), content);
  ...
  await fs.writeFile(path.join(docsDir, 'component-patterns.md'), patternsMarkdown);
  console.log('Component patterns analyzed and documented.');
}
...
function analyzeComponent(content, filePath) {
  ...
  } else if (filePath.includes('/app/') && !content.includes("'use client'") && !content.includes('"use client"')) {
...
async function generateProjectSummary() {
  ...
  try {
    ...
    markdown += `\n### UI Libraries\n\n`;
    const uiLibs = Object.keys(packageJson.dependencies || {}).filter(dep => 
      dep.includes('ui') || 
      dep.includes('radix') || 
      dep.includes('shadcn') ||
      dep.includes('chakra') ||
    ...
    markdown += `\n### State Management\n\n`;
    const stateMgmt = Object.keys(packageJson.dependencies || {}).filter(dep => 
      dep.includes('redux') || 
    ...
    for (const configFile of ['eslint.config.js', '.eslintrc.js', '.eslintrc.json']) {
      try {
        const configPath = path.join(rootDir, configFile);
        await fs.access(configPath);
        markdown += `- Using ${configFile}\n`;
    ...
    if (!eslintConfig) {
      markdown += `- No ESLint configuration found\n`;
    }
    ...
    const hasAppDir = await fs.stat(path.join(rootDir, 'app')).then(() => true).catch(() => false);
    // Check for pages directory (Pages Router)
    const hasPagesDir = await fs.stat(path.join(rootDir, 'pages')).then(() => true).catch(() => false);
    ...
    for (const dir of commonDirs) {
      const exists = await fs.stat(path.join(rootDir, dir)).then(() => true).catch(() => false);
      if (exists) {
        markdown += `- **${dir}**: Present\n`;
      }
    }
    
    await fs.writeFile(path.join(docsDir, 'project-summary.md'), markdown);
...
async function generateCompatibilityGuidelines() {
  ...
  let markdown = `# Code Generation Compatibility Guidelines\n\n`;
  markdown += `Generated: ${new Date().toLocaleString()}\n\n`;
  markdown += `This document provides guidelines for generating code that is compatible with this project.\n\n`;
  ...
  try {
    ...
    if (nextVersion.startsWith('13') || nextVersion.startsWith('14') || nextVersion.startsWith('15')) {
    ...
    if (Object.keys(packageJson.dependencies || {}).some(dep => dep.startsWith('@chakra-ui/'))) uiLibs.push('Chakra UI');
    if (Object.keys(packageJson.dependencies || {}).some(dep => dep.startsWith('@mui/'))) uiLibs.push('Material UI');
    ...
    await fs.writeFile(path.join(docsDir, 'compatibility-guidelines.md'), markdown);
    console.log('Compatibility guidelines generated.');
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    await documentConfigFiles();
    await analyzeComponentPatterns(filesByExt);
    await generateProjectSummary();
    await generateCompatibilityGuidelines();
    
    // Create index file
    let indexMarkdown = `# Project Documentation\n\n`;
    indexMarkdown += `Generated: ${new Date().toLocaleString()}\n\n`;
    indexMarkdown += `## Contents\n\n`;
    indexMarkdown += `1. [Project Summary](./project-summary.md)\n`;
    indexMarkdown += `2. [File Structure](./file-structure.md)\n`;
    indexMarkdown += `3. [Configuration](./configuration.md)\n`;
    indexMarkdown += `4. [Component Patterns](./component-patterns.md)\n`;
    indexMarkdown += `5. [Compatibility Guidelines](./compatibility-guidelines.md)\n`;
    
    await fs.writeFile(path.join(docsDir, 'index.md'), indexMarkdown);
    
    console.log('\n✅ Project documentation completed successfully!');
    console.log(`Documentation available at: ${path.relative(rootDir, docsDir)}/`);
  } catch (error) {
